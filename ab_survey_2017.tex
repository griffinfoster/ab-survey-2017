...
from that time until August 2017 for a total observing time of about 1400 hours.
...
cm$^{-3}$, which was detected while the telescope was slewing between two
sources.
\end{abstract}
...
\citep{2015AJ....150..199T}, and the GBT \citep{2017arXiv170107457C} have all
reported non-detections so far. No large surveys for \glspl{frb} have been carried out above
L-band frequencies. This is, in part, due to the narrowing of the telescope beam size which
limits survey speed. \cite{2017arXiv170507553L} carried out a coordinated-in-time,
multi-telescope campaign of the repeater \gls{frb}. They report non-detection
of pulses at VHF, C-band, Ku-band during epochs when bursts were detected at L-band
and S-band.
...
between sensitivity and beam size. ASKAP dishes with \glspl{paf} provide a
large sky coverage with enough sensitivity to detect bright \glspl{frb}. 
Interferometric arrays such as CHIME and MeerKAT provide both
sensitivity and rapid survey speed.

Till date, only one \gls{frb} has been discovered with Arecibo, FRB121102
\citep{2014ApJ...790..101S}. This is the only \gls{frb} seen to repeat during
follow-up observations \citep{2016Natur.531..202S}. The repeating nature of this
\gls{frb} has allowed follow-up observations to localize the source with the
VLA \citep{2017Natur.541...58C} and the EVN \citep{2017ApJ...834L...8M}.
Multi-wavelength observations have been carried out which indicate that the source has
broad-band emission at radio frequencies \citep{2017arXiv170507553L,atel10675}, but has
significant flux variation across the band and across epochs. 
...
commensally with the PALFA survey since 2015 these two back-ends act as
...
survey. The PALFA survey continues to do a \gls{sps} for \glspl{frb} across the full
...
was occurring it is challenging to determine accurate source position. Pointing
...
effect.  \cite{2017ApJ...842...35C} present the idea that plasma lenses in the
host galaxy of \gls{frb} progenitors can modulate the pulse amplitude as a
function of time (if the source repeats) and frequency.  If an \gls{frb} did
occur in the field of view of the telescope while ALFABURST was in operation we
could have been unlucky and scintillation caused the pulse in the band to go
below the detection threshold.  An increase to the full \gls{alfa} band would
result in a $\sqrt{6}$ increase in sensitivity, assuming no scintillation. 
...
candle model \citep{2013MNRAS.436L...5L}.  \cite{2015MNRAS.451.3278M} predict
...
% future expansion of parameter space: pulse width, bandwidth
