...
\textbf{A limited number of \gls{frb} surveys have been above L-band frequencies.
...
\textbf{Small dishes, such as those in the ATA `Fly's Eye' survey
...
\textbf{The majority of \glspl{frb} have been discovered with Parkes using the
...
model based on a trained probabilistic classifier.  \textbf{The use of trained
...
without relying on the classifier model. \textbf{Data sets were cut if any of the
...
pulsar.  \textbf{This event is similar to FRB010621 \citep{2011MNRAS.415.3065K}
...
beam. \textbf{The beam model and polynomial fits to the survey metric curves are
...
\textbf{Additionally, in a multiple beam system a bright \gls{frb} would be
...
as steep as predicted, {\bf or that the source count distribution begins to flatten below the Parkes sensitivity threshold \citep[for further discussion on FRB source counts, see][]{2017arXiv171011493M}.}
...
that the density of sources per unit co-moving volume is constant.  If {\bf FRBs are standard candles, and that} there is
...
factors that might hinder detection, {\bf and keeping in mind the standard candle assumption}, our null result suggests that the density
...
We thank the referee for constructive comments on the manuscript. ALFABURST activities are supported by a National Science Foundation (NSF) award
AST-1616042.  MAM was supported by NSF award number AST-1517003. MPS and DRL were
