...
\glspl{frb} are short-duration, broad-band, dispersed pulses that are
detected at radio frequencies. They are mostly classified by virtue of
their dispersion being far in excess of the expected Galactic
contribution. In the field of \glspl{frb}, as with radio pulsars, the dispersion measure is defined by the expression
\begin{equation}
\Delta~t = constants \times DM \times\left(\frac{1}{\nu_1^2} - \frac{1}{\nu_2^2}\right).
\end{equation}
...
eminating from point-like sources, they offer the unique opportunities
...
range from 176 pc cm$^{-3}$ (FRB170827) to 1629 pc cm$^{-3}$
(FRB121002), with pulse widths ranging from sub-ms (unresolved) to
...
are the statitistics of source numbers versus source flux density, and
...
\gls{sps} to detect pulses. As the PALFA survey is targetted at the
...
%But, scintillation effects can reduce the overall \gls{frb} event
%rates \citep{2015MNRAS.451.3278M}.
...
very broad range of detections (number!) in trial DMs are cut.
...
\subsection{The event of 2017, June 18}
\label{sec:18062017}
...
\ref{fig:D20170618_dmspace}) (SHALL WE FIT THIS?).
...
(THESE SUBSECTIONS NEED TO BE MERGED, we should not be making a distinction)
